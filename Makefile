CC?=gcc
CFLAGS?=-g -pedantic
INCLUDE_PATHS?=${PATH_FLECS_INCLUDE} ${PATH_CILENT_INCLUDE} ${PATH_FPSLOOP_INCLUDE} ${PATH_INPUT_INCLUDE} ${PATH_SPRENDER_INCLUDE} ${PATH_FNA3D_INCLUDE} ${PATH_LUA_INCLUDE} ${PATH_SDL2_INCLUDE}
LIBRARY_PATHS?=${PATH_FLECS_LIB} ${PATH_CILENT_LIB} ${PATH_FPSLOOP_LIB} ${PATH_INPUT_LIB} ${PATH_SPRENDER_LIB} ${PATH_FNA3D_LIB} ${PATH_LUA_LIB} ${PATH_SDL2_LIB}

.PHONY=build
build: lib
	${CC} ${CFLAGS} -c src/main.c -o src/main.o ${INCLUDE_PATHS}
	${CC} ${CFLAGS} src/main.o -o bin/${PLATFORM}/${BUILD}/main -lflecs -lcilent -lfpsloop -lstginput -lsprender -lFNA3D -llua54 -lm ${LIBRARY_PATHS} -Wl,-rpath=./libs/
